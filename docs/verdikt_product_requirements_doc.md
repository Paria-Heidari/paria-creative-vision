...
- [ ] Auth with magic link (Supabase Auth)
- [ ] Multi-tenant organizations (one `org_id` on all tables)
- [ ] Role-based access: Employee / Manager / Admin
- [ ] Decision CRUD (create, view, list, filter by status)
- [ ] Workflow engine: Draft → Pending → Approved / Rejected
- [ ] Rejection requires documented reason (enforced at API level)
- [ ] Audit log (append-only, records every state transition)
- [ ] Comment thread on each decision
- [ ] Analytics dashboard (6 KPIs + 2 charts)
- [ ] Seed data: 50 decisions, 5 departments, 3 user roles
- [ ] Deploy to Vercel (as part of portfolio site)
